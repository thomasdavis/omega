...
// Initialize file transfer system (resumes pending transfers)
try {
  initializeFileTransferSystem();
} catch (error) {
  console.error('⚠️  Failed to initialize file transfer system (continuing anyway):', error);
  // Don't exit - bot can still work, transfers just won't auto-resume
}
